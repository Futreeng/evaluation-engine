...
(function () {
  ...
  window.scalecraftMockFetch = async function (url, init = {}) {
    ...
    if (method === 'POST' && (m = path.match(/^\/reports\/([^/]+)\/share$/))) return json(200, { share_id: 'mock1', url: location.origin + '/s/mock1', png: { story: '', square: '' } });
    ...
    if (method === 'GET' && path === '/account/subscription-status') { const free = entitlement.current_tier === 'social_snapshot'; return json(200, { user_id: 'usr_mock', current_tier: entitlement.current_tier, tier_name: free ? 'Snapshot' : 'Growth Plan', monthly_price: free ? 0 : 12, tier_start_date: entitlement.started || (entitlement.started = Date.now()), billing_period_start: free ? null : (entitlement.period_end || Date.now()) - 30 * 86400000, email_paused: !!mockPrefs.paused, billing_period_end: free ? null : (entitlement.period_end || (entitlement.period_end = Date.now() + 30 * 86400000)), cancel_at: entitlement.cancel_at || null, status: free ? 'free' : entitlement.cancel_at ? 'cancel_pending' : 'active', email_prefs: mockPrefs }); }
