...
(function () {
  ...
  window.scalecraftMockFetch = async function (url, init = {}) {
    ...
    if (method === 'POST' && (m = path.match(/^\/reports\/([^/]+)\/context$/))) { const r = reports.get(m[1]); if (!r) return json(404, { error: 'Report not found' }); r.plan_context = { ...(r.plan_context || {}), ...(body.link !== undefined ? { link: body.link || null } : {}), ...(body.contact !== undefined ? { contact: body.contact || null } : {}) }; return json(200, { plan_context: r.plan_context }); }
