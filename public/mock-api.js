...
(function () {
  ...
  const PRICING = {
    ...
    tiers: [
      { tier: 'social_snapshot', name: 'Snapshot', description: 'See where your account stands and why.', monthlyPrice: 0, annualPrice: 0, note: 'One report per email', features: ['Your score and the four dimensions', 'Why each one landed where it did', 'Your best and worst posts', 'The first move of each phase'], cta: 'Score my account' },
      { tier: 'growth_plan', name: 'Growth Plan', description: 'Your account, re-scored every week, with the whole 90 days written from your own posts.', monthlyPrice: 12, annualPrice: 108, popular: true, features: ['All 90 days: every move, 01 through 13, with the how and a paste-ready example', 'Your 12-week posting calendar with a brief per post', 'Written around your next 90 days — time, goal, what you can shoot', 'Re-scored every week — see what each move changed', 'Day-30 and day-60 check-ins that reshape the plan if life changes', 'Up to 5 competitors, scored the same way', 'Score and follower history, and a fresh plan every 90 days'], cta: 'Start Growth Plan' },
    ...
    business: [
      { tier: 'business_growth', name: 'Business', description: 'Scored against your category; the plan is written for bookings.', monthlyPrice: 39, annualPrice: 351, features: ['One business account, scored against its category', 'The booking-led plan and calendar', 'Weekly refresh'], cta: 'Start Business Growth Plan' },
      { tier: 'business_evaluator', name: 'Business Pro', description: 'The plan answers to the P&L, not just the feed.', monthlyPrice: 99, annualPrice: 891, features: ['Up to five locations or accounts', 'Category benchmarks and competitor set', 'Priority refresh'], cta: 'Start Business Evaluator' }
    ]
  };
  ...
  window.scalecraftMockFetch = async function (url, init = {}) {
    ...
    if (method === 'POST' && (m = path.match(/^\/reports\/([^/]+)\/posts\/regenerate$/))) { const r = reports.get(m[1]); if (!r || !r.next_posts) return json(404, { error: 'Report not found' }); const i = Number(body.index) || 0; const p = { ...r.next_posts[i], hook: r.next_posts[i].hook + ' (rewritten)', written_at: Date.now() }; r.next_posts[i] = p; return json(200, { post: p, index: i }); }
    ...
    if (method === 'GET' && path === '/account/subscription-status') { const free = entitlement.current_tier === 'social_snapshot'; return json(200, { user_id: 'usr_mock', current_tier: entitlement.current_tier, tier_name: free ? 'Snapshot' : entitlement.current_tier === 'maintenance' ? 'Maintenance' : 'Growth Plan', monthly_price: free ? 0 : entitlement.current_tier === 'maintenance' ? 5 : 12, paused_until: entitlement.paused_until || null, tier_start_date: entitlement.started || (entitlement.started = Date.now()), billing_period_start: free ? null : (entitlement.period_end || Date.now()) - 30 * 86400000, email_paused: !!mockPrefs.paused, billing_period_end: free ? null : (entitlement.period_end || (entitlement.period_end = Date.now() + 30 * 86400000)), cancel_at: entitlement.cancel_at || null, status: free ? 'free' : entitlement.paused_until ? 'paused' : entitlement.cancel_at ? 'cancel_pending' : 'active', email_prefs: mockPrefs }); }
    ...
    if (method === 'POST' && path === '/billing/switch') { if (!['maintenance', 'growth_plan'].includes(body.tier)) return json(400, { error: 'Only maintenance and growth_plan can be switched to' }); entitlement.current_tier = body.tier; delete entitlement.cancel_at; return json(200, { status: 'active', currentTier: body.tier }); }
