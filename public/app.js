      ...
      <div class="wrap">
        ...
        ${paid && !isSample && report.offers && report.offers.annual && !sget('sc_annual_dismissed', false) ? raw(h`<div class="card offercard" id="annualOffer"><div class="eb">YOUR FIRST RISE · ANNUAL PLAN</div><h2>Lock in a year for $${report.offers.annual.annual}.</h2><p>Instead of $${report.offers.annual.monthly * 12} month by month — $${report.offers.annual.saves} off, the same plan, nothing changes in what runs weekly.</p><div class="acts"><button type="button" class="btn dark" data-action="take-annual">Switch to annual</button><button type="button" class="btn ghost" data-action="dismiss-annual">Not now</button></div></div>`) : ''}
          ...
          ${paid && calWeeks.length ? raw(h`<details class="card acc" open>
            ...
            <div class="body" style="gap:8px">${report.calendar?.schedule?.guessed?.length ? raw(h`<div class="fine">Posting days: ${report.calendar.schedule.label}. ${report.calendar.schedule.evidence.length ? `${report.calendar.schedule.evidence.join(' and ')} come from your own best days; ` : ''}${report.calendar.schedule.guessed.join(' and ')} ${report.calendar.schedule.guessed.length === 1 ? 'is' : 'are'} a starting point until you've posted there a few times.</div>`) : ''}${raw(calWeeks.map((w, i) => h`<details class="week" ${i === 0 ? 'open' : ''}>
              <summary><span class="wk">WEEK ${w.week} · DAYS ${(w.week - 1) * 7 + 1}–${w.week * 7}</span><span class="sl">${(w.slots || []).map(sl => `${String(sl.day).slice(0, 3)} ${sl.format}`).join(' · ')}${(w.slots || [])[0]?.angle ? raw(h` <em>— ${(w.slots || []).map(sl => sl.angle).filter(Boolean).slice(0, 2).join(' · ')}</em>`) : ''}</span></summary>
