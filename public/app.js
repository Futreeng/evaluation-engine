  ...
  async function viewReport(reportId) {
    ...
    $view.innerHTML = h`
      <div class="wrap">
        ...
        ${raw(momentsToShow.map(m => h`<div class="moment ${m.kind}" data-moment="${m.key}"><div class="t"><div class="eb">${m.kind === 'rank_up' ? 'RANK UP' : m.kind === 'record' ? 'PERSONAL RECORD' : m.kind === 'badge' ? 'BADGE' : 'MILESTONE'} · FROM THIS RE-SCORE</div><h3>${m.title}</h3><p>${m.line}</p></div><div class="acts"><button class="btn dark" data-moment-share="${m.key}">Share the card</button><button class="btn ghost" data-moment-dismiss="${m.key}">Later</button></div></div>`).join(''))}
