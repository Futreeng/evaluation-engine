...
function layout(title, inner, { footerNote } = {}) {
...
<tr><td style="padding:18px 28px 24px;border-top:1px solid #EADFCB;font-size:12px;line-height:1.7;color:#7A6A57">${footerNote ? esc(footerNote) + " · " : ""}Scalecraft · <a href="${APP}/#/reports" style="color:#7A6A57">Your reports</a> · <a href="${APP}/#/legal/privacy" style="color:#7A6A57">Privacy</a></td></tr>
...
function checkin({ to, handle, reportId, phase, phaseLabel, firstMove, doneCount, totalCount }) {
  const url = reportUrl(reportId);
  ...
  return send({ to, subject: `Phase ${phase} starts Monday — anything change?`, html: layout("Check-in", inner), tag: "checkin" });
}

// Weekly refresh where the score moved, optionally with a nudge.
function scoreChanged({ to, handle, reportId, oldScore, newScore, dimension, delta, movesDone, nudge }) {
  const url = reportUrl(reportId);
  ...
  return send({ to, subject: `${handle}: ${oldScore} → ${newScore}`, html: layout("Score changed", inner), tag: "score_changed" });
}

// Day 60 for one-time buyers: the plan they bought is over.
function planEnded({ to, handle, reportId, overall, price }) {
  const url = reportUrl(reportId);
  ...
  return send({ to, subject: `Your 60-day plan for @${handle} is done — what now?`, html: layout("Plan ended", inner), tag: "plan_ended" });
}
