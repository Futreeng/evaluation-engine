...
const button = (href, label, bg = "#B84E2A") => `<a href="${href}" style="display:block;margin-top:22px;padding:16px;border-radius:14px;background:${bg};color:#FFF6E9;text-align:center;font-size:16px;font-weight:700;text-decoration:none">${esc(label)}</a>`;
...
const moveCard = (eyebrow, action, why) => `<div style="margin-top:20px;padding:20px;background:#FFF6E9;border:1px solid #EADFCB;border-left:5px solid #B84E2A;border-radius:18px">
...
function scoreChanged({ to, userId, handle, reportId, oldScore, newScore, dimension, delta, movesDone, nudge, brief = null }) {
  ...
  const inner = h2(up ? `Your score went up.` : `Your score slipped.`) + scoreRow(oldScore, newScore)
    ...
    + (brief && brief.ready && brief.lines?.length ? `<div style="margin-top:22px;padding-top:16px;border-top:1px solid #EADFCB"><div style="font-size:11px;letter-spacing:.1em;font-weight:700;color:#B84E2A">WHAT'S WORKING IN ${esc(String(brief.category || "").replace(/_/g, " ").toUpperCase())} THIS WEEK</div><ul style="margin:8px 0 0;padding-left:18px;font-size:14px;line-height:1.55;color:#5B4C3B">${brief.lines.slice(0, 3).map((l) => `<li style="margin:0 0 6px">${esc(l)}</li>`).join("")}</ul><div style="font-size:12px;color:#7A6A57;margin-top:6px">From ${esc(brief.n)} ${esc(String(brief.category || "").replace(/_/g, " "))} accounts we've scored, aggregated and anonymised.</div></div>` : "")
