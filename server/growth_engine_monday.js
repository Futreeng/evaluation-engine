...
async function sendMondayMoves({ now = Date.now(), force = false } = {}) {
  ...
  for (const r of latest.values()) {
    ...
    if (paid) {
      ...
      if (!body.one_time_unlock && r.accountId) { try { const ent = await require("./growth_engine_entitlements").effective(r.accountId); if (!require("./growth_engine_entitlements").has(ent, "monday_move") || ent.paused) continue; } catch { continue; } }
    }
